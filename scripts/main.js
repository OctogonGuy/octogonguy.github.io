...
<nav>
  <ul>
    ...
    <li><a href="pokemon-type-game.html">Pokémon Type Game</a></li>
    <li><a href="aliens-and-asteroids.html">Aliens and Asteroids</a></li>
  </ul>
</nav>
`;
...
for (let i = 1; i <= 9; i++) {
  backgrounds.push(
    (backgrounds[backgrounds.length] = "images/background-" + i + ".png")
  );
}
...
for (let i = 1; i <= 3; i++) {
  bubble_pictures.push(
    (bubble_pictures[bubble_pictures.length] = "images/bubble-" + i + ".png")
  );
}
...
addEventListener("mousemove", (event) => {
  ...
  let bubbleDurationMidrange = parseInt(
    getComputedStyle(bubble).getPropertyValue("--bubble-duration-midrange")
  );
  let bubbleDurationRange = parseInt(
    getComputedStyle(bubble).getPropertyValue("--bubble-duration-range")
  );
  let bubbleDurationUnits = getComputedStyle(bubble)
    .getPropertyValue("--bubble-duration-midrange")
    .replace(bubbleDurationMidrange, "");
  let bubbleDuration =
    bubbleDurationMidrange + (Math.random() * 2 - 1) * bubbleDurationRange;
  ...
  bubble.style.backgroundImage =
    "url(" +
    bubble_pictures[Math.floor(Math.random() * bubble_pictures.length)];

  let minSize = parseInt(
    getComputedStyle(bubble).getPropertyValue("--min-size")
  );
  let maxSize = parseInt(
    getComputedStyle(bubble).getPropertyValue("--max-size")
  );
  let sizeUnits = getComputedStyle(bubble)
  ...
  let translateXMidrange = parseInt(
    getComputedStyle(bubble).getPropertyValue("--translate-x-midrange")
  );
  let translateXRange = parseInt(
    getComputedStyle(bubble).getPropertyValue("--translate-x-range")
  );
  let translateXUnits = getComputedStyle(bubble)
    .getPropertyValue("--translate-x-midrange")
    .replace(translateXMidrange, "");
  let translateX =
    translateXMidrange + (Math.random() * 2 - 1) * translateXRange;
  let translateYMidrange = parseInt(
    getComputedStyle(bubble).getPropertyValue("--translate-y-midrange")
  );
  let translateYRange = parseInt(
    getComputedStyle(bubble).getPropertyValue("--translate-y-range")
  );
  let translateYUnits = getComputedStyle(bubble)
    .getPropertyValue("--translate-y-midrange")
    .replace(translateYMidrange, "");
  let translateY =
    translateYMidrange + (Math.random() * 2 - 1) * translateYRange;
