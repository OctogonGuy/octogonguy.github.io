    ...
    if (this.getPosition(grid) != null) {
      for (let direction of this.validDirections) {
        ...
        if (
          ...
          grid[newRow][newCol].player == this.player
        )
          continue;
...
function addToHistory(grid) {
  ...
  if (
    gridHistory.length >
    CONSECUTIVE_MOVES_TILL_DRAW * Object.keys(Player).length * 2
  ) {
    gridHistory.pop();
  }
  ...
  for (const hisGrid of gridHistory) {
    gridHistoryCounts[hisGrid] = gridHistoryCounts[hisGrid]
      ? gridHistoryCounts[hisGrid] + 1
      : 1;
  }
}
...
function actionStart(animal, event) {
  ...
  draggingElement.classList.add(
    draggingParent.classList.contains("hand") ? "fromHand" : "fromGrid"
  );
...
function actionRelease(animal) {
  ...
  for (let i = 0; i < ROWS; i++) {
    for (let j = 0; j < COLUMNS; j++) {
      if (
        hoveredSpace == spaceGrid[i][j] &&
        isValidMove(draggingAnimal, new Position(i, j))
      ) {
...
function actionMove(event) {
  ...
  draggingAnimal.pieceElement.style.top =
    draggingAnimal.pieceElement.offsetTop + deltaY + "px";
  draggingAnimal.pieceElement.style.left =
    draggingAnimal.pieceElement.offsetLeft + deltaX + "px";
}
...
function start() {
  ...
  for (let animal of animals) {
    ...
    element.addEventListener("mousedown", (evt) => {
      actionStart(animal, evt);
    });
    element.addEventListener("touchstart", (evt) => {
      actionStart(animal, evt);
    });
    element.addEventListener("mouseup", () => {
      actionRelease(animal);
    });
...
function move(animal, toPosition) {
  ...
  if (capturedAnimal != null) {
    spaceGrid[toPosition.row][toPosition.col].removeChild(
      capturedAnimal.pieceElement
    );
  }
...
function promoteChick(animal) {
  if (animal == null || !(animal instanceof Chick && animal.type == "chick"))
    return false;
  if (animal.getPosition(animalGrid) == null) return false;
  if (
    !(
      animal.getPosition(animalGrid).row ==
      (animal.player == Player.TOP ? ROWS - 1 : 0)
    )
  )
    return false;
...
function demoteChicken(animal) {
  if (animal == null || !(animal instanceof Chick && animal.type == "chicken"))
    return false;
  if (animal.getPosition(animalGrid) != null) return false;
...
function lionInLastRank() {
  ...
  for (let col = 0; col < COLUMNS; col++) {
    if (
      animalGrid[row][col] instanceof Lion &&
      animalGrid[row][col].player == curPlayer
    ) {
...
function noCheck(lion) {
  for (let posChange of Object.values(OrdinalDirection)) {
    ...
    if (
      ...
      animalGrid[newRow][newCol].player != lion.player
    ) {
...
class Game {
  ...
  addToHistory(grid) {
    ...
    for (let i = 0; i < ROWS; i++) {
      for (let j = 0; j < COLUMNS; j++) {
        if (grid[i][j] != null) gridCopy[i][j] = grid[i][j].clone();
        else {
          gridCopy[i][j] = null;
        }
      }
  ...
  numPieces(player) {
    ...
    for (const animal of this.animals) {
      if (animal.player === player) {
        if (animal instanceof Lion) numPieces += 100;
        else if (animal instanceof Chick && animal.type === "chicken")
          numPieces += 18;
        else if (animal instanceof Chick) numPieces += 3;
        else if (animal instanceof Giraffe) numPieces += 12;
        else if (animal instanceof Elephant) numPieces += 12;
      }
    }
    return numPieces;
  }
  ...
  promoteChick(animal) {
    ...
    if (
      !(
        animal.getPosition(this.animalGrid).row ==
        (animal.player == Player.TOP ? ROWS - 1 : 0)
      )
    )
      return false;
  ...
  demoteChicken(animal) {
    if (
      animal == null ||
      !(animal instanceof Chick && animal.type == "chicken")
    )
      return false;
    if (animal.getPosition(this.animalGrid) != null) return false;
  ...
  lionInLastRank() {
    ...
    for (let col = 0; col < COLUMNS; col++) {
      if (
        this.animalGrid[row][col] instanceof Lion &&
        this.animalGrid[row][col].player === Player.BOTTOM
      ) {
    ...
    for (let col = 0; col < COLUMNS; col++) {
      if (
        this.animalGrid[row][col] instanceof Lion &&
        this.animalGrid[row][col].player === Player.TOP
      ) {
        return this.animalGrid[row][col];
      }
    }
  ...
  noCheck(lion) {
    for (let posChange of Object.values(OrdinalDirection)) {
      ...
      if (newCol < 0 || newCol >= COLUMNS) continue;
      if (
        this.animalGrid[newRow][newCol] != null &&
        this.animalGrid[newRow][newCol].player != lion.player
      ) {
        for (let position of this.animalGrid[newRow][
          newCol
        ].getValidMovePositions(this.animalGrid)) {
...
const DEFAULT_MAX_DEPTH = 5; // Default maximum search depth for minimax
let maxDepth; // Maximum search depth depending on difficulty
...
function randomMove(game) {
  const move = game.validMoves(game.curPlayer)[
    randomInteger(game.validMoves(game.curPlayer).length)
  ];
...
function minimaxMove(game) {
  ...
  for (let move of game.validMoves(movedPlayer)) {
    ...
    value = minimax(gameCopy, maxDepth - 1, false);
    move = [frompos, move[1]];
...
function staticEvaluation(game, maximizingPlayer) {
  ...
  if (game.capturedLion() != null) {
    if (maximizingPlayer) return Number.MIN_SAFE_INTEGER;
